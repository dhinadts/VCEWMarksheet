...
def test_bad_password_is_rejected(client):
    response = client.post("/api/v1/auth/login", json={"username": "ADMIN01", "password": "wrong"})
    assert response.status_code == 401


def test_refresh_token_rotates_and_cannot_be_reused(client):
    login_response = client.post("/api/v1/auth/login", json={"username": "ADMIN01", "password": "1234"}).json()
...
def test_student_password_login_is_not_allowed(client):
    response = client.post("/api/v1/auth/login", json={"username": "DEMOSTU01", "password": "1234"})
