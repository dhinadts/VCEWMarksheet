...
def seed(db: Session) -> dict:
    ...
    professors = []
    for index, name in enumerate(PROFESSOR_NAMES[:1], 1):
    ...
    class5 = one(db, AcademicClass, code="BT-VI-A", defaults={"academic_year_id": year.id, "department_id": department.id, "programme_id": programme.id, "semester_id": sem6.id, "section": "A", "batch": "2024-2028", "strength": 10, "class_advisor_id": professors[0].id})
    class3.strength = 0
    class5.strength = 10
    student_password_hash = hash_password(password)
    for index in range(1, 11):
        roll_number = f"DEMOSTU{index:02d}"; semester_no = 6; assigned_class = class5
        ...
        user.password_hash = student_password_hash; user.is_active = True; user.must_change_password = False
    ...
    db.query(User).filter(User.username.like("VCEW%")).update({User.is_active: False}, synchronize_session=False)
    db.query(User).filter(User.username.like("PROF%"), User.username != "PROF01").update({User.is_active: False}, synchronize_session=False)
    db.query(User).filter(User.username.like("ADMIN%"), User.username != "ADMIN01").update({User.is_active: False}, synchronize_session=False)
    ...
    db.commit()
    return {"users": db.scalar(select(__import__('sqlalchemy').func.count()).select_from(User).where(User.is_active.is_(True))), "students": 10, "professors": 1, "admins": 1, "courses": 1}
